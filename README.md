...
`~/Downloads/ark-kontrol-listesi.html` dosyasındaydı (S0: marka/altyapı, S1: lead makinesi,
S2: şablon/vitrin, S3: bot/reklam/CRM, S4: tam otomasyon).
...
- [x] `litellm/` router + tüm API key'ler `.env`'de (repoya girmez, `.gitignore` ile korunuyor)
- [x] `bexi-app/` Next.js iskeleti (henüz sohbet arayüzü/API route yazılmadı)
- [ ] Supabase projesi (sectors, businesses, audits, leads, conversations tabloları) — henüz kurulmadı
- [ ] OpenRouter hesabına DeepSeek V4 Pro için bakiye yüklenmesi
- [ ] `oh-my-claudecode` + ccr uyumluluk testi (Ajan 3 için, henüz yapılmadı)
- [ ] Bexi'nin gerçek WhatsApp/telefon PWA entegrasyonu
